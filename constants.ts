...
// export const PERIOD_CONFIG: Record<
//   Period,
//   { days: number | string; interval: 'minute' | 'hourly' | 'daily' }
// > = {
//   daily: { days: 1, interval: 'minute' }, // 1-day data → use minute interval
//   weekly: { days: 7, interval: 'hourly' }, // 7-day → hourly
//   monthly: { days: 30, interval: 'hourly' }, // 30-day → hourly
//   '3months': { days: 90, interval: 'hourly' }, // 90-day → hourly
//   '6months': { days: 180, interval: 'daily' }, // 180-day → daily
//   yearly: { days: 365, interval: 'daily' }, // 1 year → daily
//   max: { days: 'max', interval: 'daily' }, // max → daily
// };

export const PERIOD_CONFIG: Record<Period, { days: number }> = {
  daily: { days: 1 }, // 1-day data → use minute interval
  weekly: { days: 7 }, // 7-day → hourly
  monthly: { days: 30 }, // 30-day → hourly
  '3months': { days: 90 }, // 90-day → hourly
  '6months': { days: 180 }, // 180-day → daily
  yearly: { days: 365 }, // 1 year → daily
  // max: { days: 1095 }, // max → daily
};
